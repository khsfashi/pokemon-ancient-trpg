...
The broad social analogy is closer to a fantasy world where dangerous monsters exist throughout the wilderness but people who successfully travel with a monster are rare. A person accompanied by a dangerous Pokémon can be remarkable; someone trusted by several powerful Pokémon can be exceptional; someone known to ride or travel with a dragon-like or legendary-scale creature may be spoken of as a legendary figure.

That analogy defines the **social rarity and prestige** of unusual bonds. It does not replace species-specific Pokémon ecology with generic fantasy-monster rules.
...
There is no portable containment mechanism. If a Pokémon becomes a companion, it remains physically present: it travels, rests, eats, occupies space, can be injured or separated, and may change how strangers or settlements react to the player.

Companionship means the Pokémon has accepted an ongoing relationship and continues to remain with the person. Defeating, trapping, restraining, cornering, or temporarily transporting a Pokémon does **not** by itself create a companion.

### 7.1 Companion system invariant

Pokémon companionship is a **core game system** even though it is not required for progression.

The player has exactly three companion slots:

- `0/3` — no companions;
- `1/3` — one companion;
- `2/3` — two companions;
- `3/3` — full companion capacity.

All four states are normal supported states. Empty slots are not failures, warnings, or incomplete objectives.

The eventual mobile UI must visibly preserve these three slots so companionship remains legible as a major system even during a human-only run.

The three-slot limit is a gameplay capacity, not a statement that three companions are ordinary in the fiction. One stable bond can already be socially notable. Two or three companions should generally represent increasingly exceptional circumstances, subject to species-specific P4 rules.
...
3. Pokémon companionship is a first-class player system with exactly three slots, while `0/3` is a fully valid and completable run state.
4. Companions physically exist in the world; there is no abstract containment/storage mechanism.
